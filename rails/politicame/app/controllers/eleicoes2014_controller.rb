# Copyright 2014 de PoliticaMe/Cassio Paixao e Max Rosan.
...
class Eleicoes2014Controller < ApplicationController
  def index
    @presidenciaveis = Presidenciavel.build_all
  end

  def show
    @presidenciavel = Presidenciavel.build(params[:id])
    redirect_to :index if @presidenciavel.nil?
    render @presidenciavel.slug, :layout => false
  end
