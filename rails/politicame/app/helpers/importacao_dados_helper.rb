...
module ImportacaoDadosHelper
  ...
  def tratar_votacao(xml_body, args)
    ...
    v.xpath('./votos/Deputado').each do |voto|
      ...
      deputado_id_camara = voto.xpath('./@ideCadastro').first.content.strip.to_i

      dep = Deputado.where(:id_camara => deputado_id_camara).first
      voto_deputado[:deputado_id] = dep.id if dep.present?
